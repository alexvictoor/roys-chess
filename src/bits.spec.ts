...
describe("64 bits", () => {
  it("should store booleans true", () => {
    // given
    const randomIndexs = [0, 17, 26, 32, 42, 63];
    const bits = new Bits();
    // when
    randomIndexs.forEach((idx) => bits.set(idx, true));
    // then
    for (let index = 0; index < 64; index++) {
      expect(bits.get(index)).toBe(randomIndexs.includes(index));
    }
  });
  ...
  it("should store booleans false", () => {
    ...
    // when
    randomIndexs.forEach((idx) => bits.set(idx, true));
    randomIndexs.forEach((idx) => bits.set(idx, false));
    // then
    for (let index = 0; index < 64; index++) {
